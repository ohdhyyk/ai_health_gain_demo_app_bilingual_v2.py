...
# ------------------------
# CSS for unified, light, compact design
# ------------------------
st.markdown(
    """
    <style>
...
S = {
    "EN": {
        ...
        "see_details": "See model details (demo)",
    ...
    "NO": {
        ...
        "see_details": "Se modell-detaljer (demo)",
...
<span class='highlight-number'>🌿 +{alcohol_gain:.1f} months on average</span>",
    ...
    txt = f"Alcohol module result: +{alcohol_gain} months"
    st.download_button(S[LANG]["download_txt"], txt, file_name="alcohol_result.txt")

    df = pd.DataFrame([{ "alcohol_gain": alcohol_gain }])
    csv_buf = io.StringIO()
    df.to_csv(csv_buf, index=False)
    st.download_button(S[LANG]["download_csv"], csv_buf.getvalue(), file_name="alcohol_result.csv")
...
<span class='highlight-number'>🌿 +{smoking_gain} months on average</span>",
        unsafe_allow_html=True,
    )
    ...
    csv_buf_s = io.StringIO()
    df_s.to_csv(csv_buf_s, index=False)
    st.download_button(S[LANG]["download_csv"], csv_buf_s.getvalue(), file_name="smoking_result.csv")
