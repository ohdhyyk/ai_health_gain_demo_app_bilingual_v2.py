...
# ------------------------
# Minimal CSS for modern look and highlight style
# ------------------------
st.markdown(
    """
    <style>
...
def health_gain_alcohol(drinking_days, target_days):
    return max(0, (drinking_days - target_days) * 0.8)  # months

def health_gain_smoking(cigs_now, cigs_goal):
    return max(0, (cigs_now - cigs_goal) / 20 * 96)  # 20→0 = +8y = 96 months
...
if submitted:
    ...
    # Alcohol result with visual emphasis
    st.markdown(
        f"""
        **Nice move!**  
    ...
    # Smoking result with visual emphasis
    st.markdown(
        f"""
        **Strong choice!**  
